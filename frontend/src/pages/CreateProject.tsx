  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === "Network Error") {
        setError(
          `Unable to reach backend. Check that the server is running and VITE_API_URL is set correctly (${apiUrl || "/api"}).`,
        );
      } else {
        setError(
          err.response?.data?.detail || err.response?.data?.error || err.message || "Failed to create project"
        );
      }
    } finally {
      setLoading(false);
