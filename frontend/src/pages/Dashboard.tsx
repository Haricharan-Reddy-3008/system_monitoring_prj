...
import { Activity, AlertTriangle, Cpu, Database, Layout, RefreshCw, Terminal } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchPrediction = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/predictions/${projectId}`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-xl overflow-hidden relative">
            <div className="absolute top-0 right-0 p-3 text-blue-500/20">
              <Cpu className="w-12 h-12" />
            </div>
            <p className="text-sm font-medium text-slate-400 mb-1 uppercase tracking-wider">CPU Usage</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{metrics[0]?.cpu?.toFixed(1) || '0'}%</span>
              <span className={`text-xs ${metrics[0]?.cpu > (metrics[1]?.cpu || 0) ? 'text-rose-400' : 'text-emerald-400'}`}>
          ...
          <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-xl overflow-hidden relative">
            ...
            <p className="text-sm font-medium text-slate-400 mb-1 uppercase tracking-wider">Memory</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{metrics[0]?.memory?.toFixed(1) || '0'}%</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-xl overflow-hidden relative">
            ...
            <p className="text-sm font-medium text-slate-400 mb-1 uppercase tracking-wider">Requests / min</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{metrics[0]?.requests || '0'}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-xl overflow-hidden relative">
            ...
            <p className="text-sm font-medium text-slate-400 mb-1 uppercase tracking-wider">Errors (5m)</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{metrics.slice(0, 5).reduce((acc, curr: any) => acc + curr.errors, 0)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-slate-900 rounded-xl border border-slate-800 shadow-xl flex flex-col">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h3 className="font-bold flex items-center gap-2">
                <RefreshCw className="w-4 h-4 text-blue-400" />
                AI Predictions
              </h3>
              <span className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded uppercase font-bold tracking-tighter shadow-sm border border-blue-500/20">GEMINI</span>
            </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-xl">
              <div className="p-4 border-b border-slate-800 flex items-center justify-between">
                ...
                <Link to="#" className="text-xs text-blue-400 hover:underline">View All</Link>
              </div>
